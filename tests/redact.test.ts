...
test("redact: returns input unchanged when no entities found", () => {
  const result = Duckling().redact("Hello world, no entities here.");
  assertEquals(result, "Hello world, no entities here.");
});

test("redact: masks all entities by default (no opts)", () => {
...
test("redact: default mask character is █", () => {
...
test("redact: custom mask character", () => {
  const result = Duckling([SSN.parser]).redact("SSN: 123-45-6789", {
    mask: "X",
  });
...
test("redact: mask with asterisk", () => {
...
test("redact: filters by kinds — only SSN", () => {
...
test("redact: filters by kinds — only email", () => {
  const input = "SSN 123-45-6789, email a@b.com";
...
test("redact: filters by multiple kinds", () => {
...
test("redact: empty kinds array redacts nothing", () => {
...
test("redact: masks IP addresses", () => {
...
test("redact: empty input", () => {
  const result = Duckling().redact("");
  assertEquals(result, "");
});

test("redact: multiple entities of same type", () => {
...
test("redact: preserves surrounding whitespace and punctuation", () => {
  const result = Duckling([SSN.parser]).redact("(SSN: 123-45-6789).");
...
test("redact: kinds + mask combined", () => {
  const input = "SSN 123-45-6789, email a@b.com";
...
test("redact: with custom parser subset", () => {
...
test("redact: result length matches input length with single-char mask", () => {
